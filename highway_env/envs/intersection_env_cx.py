...
class IntersectionEnvCX1(AbstractEnv):
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        # Challenger vehicle
        self._spawn_vehicle(10, spawn_probability=1, go_straight=True, position_deviation=0.0, speed_deviation=0.2)
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            ego_vehicle = self.action_type.vehicle_class(
                             self.road,
                             ego_lane.position(80 + 5*self.np_random.normal(0), 0),
    ...
    def _spawn_vehicle(self,
        ...
        for v in self.road.vehicles:
            if np.linalg.norm(v.position - vehicle.position) < 15:
                return
        vehicle.plan_route_to("o" + str(route[1])) #NPC直行
...
class MultiAgentIntersectionEnv(IntersectionEnvCX1):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
...
class ContinuousIntersectionEnv(IntersectionEnvCX1):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
