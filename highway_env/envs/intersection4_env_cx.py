...
class IntersectionEnvCX4(AbstractEnv):
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        vehicle_type.DISTANCE_WANTED = 6.0  # Low jam distance
        vehicle_type.COMFORT_ACC_MAX = 6
        vehicle_type.COMFORT_ACC_MIN = -3
        vehicle_type.ACC_MAX = 3
        vehicle_type.TIME_WANTED = 0.4
        ...
        # Challenger vehicle
        self._spawn_vehicle(longitudinal=20, spawn_probability=1.0, go_straight=False, position_deviation=0.0, speed_deviation=0.0)
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            ego_vehicle = self.action_type.vehicle_class(
                             self.road,
                             ego_lane.position(50 + 5*self.np_random.normal(0), 0),
                            #  speed=ego_lane.speed_limit,
                             speed=0,
                             heading=ego_lane.heading_at(60))
            ego_vehicle.MIN_SPEED = 0
            try:
            ...
            for v in self.road.vehicles:  # Prevent early collisions
                if v is not ego_vehicle and np.linalg.norm(v.position - ego_vehicle.position) < 5:
...
class MultiAgentIntersectionEnv(IntersectionEnvCX4):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
...
class ContinuousIntersectionEnv(IntersectionEnvCX4):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
