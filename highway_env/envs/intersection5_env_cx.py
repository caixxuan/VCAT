...
class IntersectionEnvCX5(AbstractEnv):
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        vehicle_type.DISTANCE_WANTED = 0.5  # Low jam distance
        vehicle_type.COMFORT_ACC_MAX = 0.05
        vehicle_type.COMFORT_ACC_MIN = -0.05
        vehicle_type.ACC_MAX = 0.1

        # Random vehicles
        simulation_steps = 3
        for t in range(n_vehicles - 1):
            # self._spawn_vehicle(np.linspace(0, 80, n_vehicles)[t])
            self._spawn_vehicle(np.linspace(0, 0, n_vehicles)[t], position_deviation=np.linspace(0, 0, n_vehicles)[t])
        for _ in range(simulation_steps):
            [(self.road.act(), self.road.step(1 / self.config["simulation_frequency"])) for _ in range(self.config["simulation_frequency"])]

        # Challenger vehicle
        self._spawn_vehicle(longitudinal=10, spawn_probability=1.0, go_straight=False, position_deviation=0.0, speed_deviation=0.1)
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            ego_vehicle = self.action_type.vehicle_class(
                             self.road,
                             ego_lane.position(100 + 5*self.np_random.normal(0), 0),
                            #  speed=ego_lane.speed_limit,
                             speed=0,
                             heading=ego_lane.heading_at(60),
                             )
            ego_vehicle.LENGTH = 0.1
            ego_vehicle.WIGTH = 0.2
            ego_vehicle.MAX_SPEED = 1
            ego_vehicle.MIN_SPEED = -1
            try:
    ...
    def _spawn_vehicle(self,
                       longitudinal: float = 20,
                       position_deviation: float = 0.,
                       speed_deviation: float = 0.,
        ...
        vehicle = vehicle_type.make_on_lane(self.road, ("o" + str(route[0]), "ir" + str(route[0]), 0),
                                            ...
                                            speed = 6 + self.np_random.normal() * speed_deviation)
        for v in self.road.vehicles:
            if np.linalg.norm(v.position - vehicle.position) < 1:
                return
...
class MultiAgentIntersectionEnv(IntersectionEnvCX5):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
...
class ContinuousIntersectionEnv(IntersectionEnvCX5):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
